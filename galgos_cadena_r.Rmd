...
analisis_modelos_superlearner <- function(matrizentrada){
  ...
  print( paste( "y_train=",nrow(y_train), "x", ncol(y_train) ) )
  print( paste( "x_test=",nrow(x_test), "x", ncol(x_test) ) )
  print( paste( "y_test=",nrow(y_test), "x", ncol(y_test) ) )
  ...
  print('Review the distribution of the best single learner as external CV folds:')
...
validarModelo <- function(modeloEntrenado, x_test, y_test, tipo){
  ...
  print('-------------- VALIDACION ---------------')
  print('Usando dataset de test para validar el modelo y obtener porcentaje de acierto...')
  print(paste('Tipo: ', tipo))
  print( paste("VALIDACION-INPUTS:", "  x_test=",nrow(x_test), "x", ncol(x_test),  "   y_test=",length(y_test) ) )
  
  if (tipo == "MULTICORE_CON_CV") {
    ...
    print("modeloEntrenado$SL.predict (muestro 10 filas):")
    head(y_test_predicho_objeto)
    
    print("Coeficientes por algoritmo y fold (muestro 10 filas):")
    modeloEntrenado$coef
    
    print("Pintando plot...")
    plot(modeloEntrenado, package = "ggplot2", constant = qnorm(0.975), sort = TRUE) 
