...
attach(pasado_f)
names(pasado_f)
paste("FEATURE MATRIX: ", nrow(pasado_f), "x", ncol(pasado_f))
```
#### Targets (INPUT)
```{r}
...
attach(pasado_t)
names(pasado_t)
paste("TARGET MATRIX: ", nrow(pasado_t), "x", ncol(pasado_t))
```
...
analisis_modelos_superlearner <- function(matrizentrada){
  ...
  modelo_super <- SuperLearner(Y=y_train, X=x_train, SL.library=list("SL.svm", "SL.randomForest") )
