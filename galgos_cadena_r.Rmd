...
analisis_modelos_superlearner <- function(matrizentrada){
  ...
  names(algoritmosPredictivosUsados)
  ...
  system.time({
    modelo_multicore_con_cv <- CV.SuperLearner(Y = y_train, X = x_train, family = gaussian(), 
                          ...
                          cvControl = list(V = num_v), saveAll=T)
  })
...
#'
#' @examples
validarModelo <- function(modeloEntrenado, x_text, y_test, tipo){
  ...
  if (tipo == "UNICORE_CON_CV") {
    ...
    print(paste('El modelo usó CV con v: ', num_v))
    
    y_test_predicho_objeto <- predict( modeloEntrenado$AllSL[[num_v]] )$pred
    ...
    #Graficos de la prediccion
    library(ggplot2)
    qplot(y_test_predicho_objeto[, 1]) + theme_minimal() #Histograma de los valores predichos
    qplot(y_test, y_test_predicho_objeto[, 1]) + theme_minimal() #Scatterplot
